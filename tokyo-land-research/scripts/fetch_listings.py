...
DEV_SPOTS = [
    ...
    ("渋谷区桜丘町", "渋谷 桜丘口(SAKURA STAGE)", "再開発", 3, "渋谷駅桜丘口の大規模複合が2023竣工。渋谷駅『100年に1度』再開発の一角で駅直結・資産性上昇"),
    ("渋谷区道玄坂", "道玄坂・渋谷二丁目西",       "再開発", 3, "道玄坂通/二丁目西地区の再開発が連続。渋谷駅至近で回遊性・資産性が上昇"),
    ("渋谷区宇田川町","宇田川町・公園通り",         "再開発", 2, "公園通り〜宇田川の更新。渋谷の北側回遊が強化"),
    ("渋谷区神南",   "神南・宇田川(渋谷)",         "再開発", 2, "神南一丁目ほか渋谷駅北側の更新余地"),
    ("目黒区目黒",   "目黒駅前",                   "再開発", 2, "目黒駅前(権之助坂・三田)の更新。駅前の商業・住宅が刷新へ"),
    ("新宿区西新宿", "新宿駅西口(小田急・メトロ)", "再開発", 3, "小田急百貨店建替えの超高層＋地下歩行者網(2029頃)。新宿の玄関口が刷新"),
    ("新宿区歌舞伎町","歌舞伎町(東急歌舞伎町タワー)","再開発", 2, "歌舞伎町一丁目の高層化。エンタメ拠点で集客・地価を牽引"),
    ("港区麻布台",   "麻布台ヒルズ",               "再開発", 3, "日本一の高さの大規模複合(2023開業)。麻布・虎ノ門一帯の地価を牽引"),
    ("港区六本木",   "六本木五丁目西",             "再開発", 3, "六本木五丁目西地区の大規模再開発(2020年代後半)。麻布・六本木の更新"),
    ("港区虎ノ門",   "虎ノ門ヒルズ周辺",           "再開発", 3, "虎ノ門ヒルズ各棟＋新駅。国際新都心として最上位級"),
    ("港区芝浦",     "芝浦一丁目(浜松町)",         "再開発", 3, "芝浦一丁目の超高層ツイン。竹芝・浜松町の水辺更新"),
    ("港区高輪",     "高輪ゲートウェイシティ",     "再開発", 3, "高輪ゲートウェイの大規模複合(2025〜)。品川・車両基地跡と一体で将来性最上位"),
    ("千代田区大手町","大手町・常盤橋(TOKYO TORCH)","再開発", 3, "日本一高いTorch Tower(2028頃)。丸の内・大手町の頂点"),
    ("千代田区神田","神田・神保町",               "再開発", 2, "神田駅周辺・神保町の更新。大手町波及で商業刷新"),
    ("中央区日本橋", "日本橋(首都高地下化)",       "再開発", 3, "首都高地下化＋室町/一丁目の連続再開発。空と川が戻り資産性上昇"),
    ("中央区八重洲", "八重洲(東京駅前)",           "再開発", 3, "八重洲一・二丁目のツインタワー＋バスターミナル。東京駅前が刷新"),
    ("豊島区南池袋", "南池袋(西口・公園整備)",     "再開発", 3, "池袋駅西口再開発＋南池袋の更新・公園整備(2030年代)。副都心の刷新"),
    ("品川区大崎",   "大崎・五反田",               "再開発", 2, "大崎駅周辺の業務・住宅更新＋五反田バレー。山手線南側の実需厚い"),
    ...
    ("品川区北品川", "北品川（品川新駅周辺）", "鉄道", 3, "品川駅北・車両基地跡＋リニアで交通結節が強化。将来性最上位級"),
    ("大田区羽田",   "羽田イノベーションシティ","再開発", 2, "羽田空港跡地(HICity)の複合開発。産業・観光の新拠点"),
    # --- 波の前夜 追加（都心寄り・不燃化特区/木密の割安余地） ---
    ("渋谷区本町",   "本町(渋谷・不燃化特区)", "波", 2, "渋谷区本町の不燃化特区(木密)。初台・幡ヶ谷至近で新宿近い割安余地"),
    ("新宿区大久保", "大久保(新宿・木密)",     "波", 2, "大久保・百人町の密集市街地。新宿至近で更新余地"),
    ("中野区弥生町", "弥生町(中野・不燃化特区)","波", 2, "弥生町三丁目は不燃化特区。新宿至近の割安余地"),
    ("中野区南台",   "南台(中野・不燃化特区)", "波", 2, "南台四丁目ほか不燃化特区。方南町・新中野至近"),
    ("品川区東大井", "東大井(大井町波及)",     "波", 2, "大井町再開発の波及。木密の更新余地で実需厚い"),
    ("大田区大森西", "大森西(不燃化特区)",     "波", 2, "大森西の不燃化特区(木密)。京急・JR大森至近"),
    ("北区志茂",     "志茂(赤羽・不燃化特区)", "波", 2, "志茂の不燃化特区。赤羽再開発の波及"),
    ("足立区関原",   "関原・本木(不燃化特区)", "波", 2, "関原・本木は木密の不燃化特区。西新井・梅島至近"),
    ("板橋区仲宿",   "仲宿(板橋・不燃化特区)", "波", 2, "仲宿・大山周辺の不燃化特区。大山再開発に近接"),
    ("杉並区高円寺", "高円寺南(不燃化特区)",   "波", 2, "高円寺南の木密。中央線人気で実需厚い"),
]
...
def render(rows, errors):
    ...
    # 学び③：波の前夜エリア（地区スポット）。再開発／鉄道／不燃化特区の3カテゴリに細分化して表示。
    spot_cnt = Counter(r["spot"] for r in rows if r.get("spot"))
    GROUPS = [("🏗 再開発 直撃／近接（進行中・計画）", "再開発", "s-dev"),
              ("🚇 鉄道計画（新駅・延伸＝沿線の地価上昇期待）", "鉄道", "s-wave"),
              ("🌊 波の前夜（不燃化特区・木密＝今は割安・将来更新）", "波", "s-wave")]
    sm = []
    for gtitle, gkind, scls in GROUPS:
        spots = [s for s in DEV_SPOTS if s[2] == gkind]
        if not spots:
            continue
        cells = ""
        for pref, label, kind, st, note in sorted(spots, key=lambda s: -s[3]):
            c = spot_cnt.get(label, 0)
            stars = f'<span class="devb d{st}">{"★" * st}{"☆" * (3 - st)}</span>'
            cells += (f'<div class="spt {scls}">'
                      f'<b>{H.escape(label)}</b>{stars}'
                      f'<span class="dvn">{H.escape(note)}</span>'
                      f'<small>{("掲載" + str(c) + "件") if c else "現在は該当物件なし"}</small></div>')
        sm.append(f'<div class="wsub">{gtitle}（{len(spots)}地区）</div>'
                  f'<div class="spotgrid">{cells}</div>')
    spotmap = "".join(sm)
...
<style>
  ...
  *{{box-sizing:border-box}}
  ...
  .sharerow .sharebtn.alt{{background:#8a94a0}}
  ...
  .hit .tg{{background:#eef1f5;color:#5d6b7a;border-radius:5px;padding:0 5px;font-size:.7rem;margin-left:2px}}
...
<p class="lead" style="margin:0 0 10px">🏠 <b>1階を貸して住居費を下げる／ローンを早く返す</b>という買い方を検討するなら → <a href="./land_path.html">都心の土地までの経路</a>・<a href="./hybrid.html">一階賃貸（住居）</a>・<a href="./hybrid_tenant.html">一階テナント（店舗）</a>・<a href="./hybrid_rentout.html">住まず全部貸す</a>・<a href="./loan_plan.html">取得力と返済加速</a>／考え方は <a href="./rental_hybrid.md">rental_hybrid.md</a></p>
...
<p><b>■ 採点の軸（資産スコア0-100）</b><br>①値持ち（名門立地・出口の堅さ）／②割安度（相場比）／③駅近／④将来性＝再開発の収益upside／⑤現地解像度／⑥<b>値上がり実測</b>（実取引の区㎡単価トレンド＝年率）。<br><b>プリセット</b>と<b>注目エリアタブ</b>で戦略別に絞り込み、📌から<b>好みを学習</b>して似た物件を上位表示。</p>
<p><b>■ 公的データで“勘”を裏取り</b><br>主観でなく<b>国交省・不動産情報ライブラリ（実成約）</b>と<b>国土地理院</b>で各物件を補強：</p>
<ul class="concept-l">
<li><b>町名の実成約レンジ・町名プレミアム・値上がり率</b>（実取引）＝区平均でなく“その町の実勢”で割安/割高を判断</li>
<li><b>用途地域・容積率</b>（都市計画GIS）＝建替え/新築プロフォーマ（戦略③④）を正確に</li>
<li><b>将来推計人口</b>（社人研・地点250m）＝10〜30年の将来需要＝値持ちの最良シグナル</li>
<li><b>災害ハザード</b>（洪水浸水深・土砂警戒区域・低地地形）＝出口/融資/保険のリスク</li>
<li>さらに<b>管理・修繕の健全性／売り急ぎ・相続サイン／手取り試算（相場で売却時の往復コスト差引）</b>も自動表示</li>
</ul>
...
<div class="dbody">
<p class="lead">住所（丁目）単位の“波の前夜”スポットを<b>3カテゴリに細分化</b>して表示：<b>🏗再開発 直撃／近接</b>（渋谷桜丘・麻布台ヒルズ・虎ノ門・大手町TOKYO TORCH・日本橋・八重洲・新宿西口 等）／<b>🚇鉄道計画</b>（品川リニア・南北線品川延伸・有楽町線延伸・臨海地下鉄・大江戸線延伸・蒲蒲線 等）／<b>🌊不燃化特区・木密</b>（今は古家が多く割安だが将来更新が進む地区）。★は将来性の目安。物件カードにも該当メモを表示します。</p>
{spotmap}
<p class="lead" style="margin-top:10px">出典：東京都都市整備局「不燃化特区」各区の取組／各区・東京都の市街地再開発／鉄道各社・国交省の延伸事業。<b>★は将来性の目安（3=直撃/大規模, 2=近接/波及, 1=構想・不透明）。🚇鉄道計画は構想〜事業中で時期未確定のものを含む</b>。具体の進捗・範囲は必ず公式で確認を。</p>
</div></details>
...
<details><summary>💴 いくらまで買える？ — 世帯年収1,400万の借入・返済目安</summary>
<div class="dbody">
<p class="lead"><b>返済負担率＝年間返済額 ÷ 年収</b>。<b>25%以内が安全圏</b>、無理のない購入価格の目安は<b>借入1億（＋頭金）前後</b>まで。審査上は35%（≒借入1.4億）まで通ることもありますが、金利上昇に弱くなります。</p>
<div class="tblwrap-g"><table class="kobai">
<thead><tr><th>借入額</th><th>月返済<br>変動0.8%</th><th>月返済<br>固定2.0%</th><th>返済負担率<br>(対1,400万)</th><th>目安</th></tr></thead>
<tbody>
<tr><td>6,000万</td><td>16.4万</td><td>19.9万</td><td>14%</td><td>◎かなり余裕</td></tr>
<tr><td>8,000万</td><td>21.8万</td><td>26.5万</td><td>19%</td><td>◎無理なし</td></tr>
<tr><td><b>1.0億</b></td><td><b>27.3万</b></td><td><b>33.1万</b></td><td><b>23%</b></td><td><b>○標準・推奨上限の目安</b></td></tr>
<tr><td>1.1億</td><td>30.0万</td><td>36.4万</td><td>26%</td><td>△やや背伸び</td></tr>
<tr><td>1.2億</td><td>32.8万</td><td>39.8万</td><td>28%</td><td>△チャレンジ(変動前提)</td></tr>
<tr><td>1.4億</td><td>38.2万</td><td>46.4万</td><td>33%</td><td>▲審査上限近辺・金利上昇に弱い</td></tr>
</tbody></table></div>
<ul>
<li><b>安全圏（返済負担率〜20%）＝借入 8,000万まで</b>（月20万前後）。教育費・老後も両立しやすい。</li>
<li><b>標準（20〜25%）＝借入 8,000万〜1.0億</b>（月22〜27万）。当ツールの予算上限1億はここ。</li>
<li><b>背伸び（25〜30%）＝借入 1.0〜1.2億</b>（月27〜33万）。共働き継続・繰上返済が前提。</li>
<li><b>審査上限（30〜35%）＝借入 1.2〜1.4億</b>。通っても変動金利の上昇余地に注意。</li>
</ul>
<p class="lead"><b>年齢の影響</b>：多くの銀行は<b>完済時年齢80歳が上限</b>（借入最長＝80−年齢）。<b>39歳（1987年生）なら35年フルローン可</b>（完済74歳）＝期間・枠は満額でマイナス影響なし。<b>45歳を超えると最長期間が縮み始める</b>ので、フル35年で最大化するなら44〜45歳までに判断が有利。ただし完済74歳＝定年後も返済が残るため<b>繰上返済・退職金・年金</b>を前提に。</p>
<p class="lead">※前提：35年・元利均等・ボーナス返済なし・金利は2026年の目安（変動0.8%／固定2.0%）。<b>物件価格＝借入＋頭金</b>で、頭金を入れれば同じ物件でも借入・月返済は下がります。別途<b>諸費用（物件価格の約7〜8%）</b>が必要。ペアローン/収入合算なら借入枠は増えますが、離職・金利上昇時の耐性は要検討。最終は金融機関の事前審査で確認を。</p>
</div></details>
...
function fbPush(){{
  if(!syncCode)return;
  fbBusy=true;
  fetch(fbPath(syncCode),{{method:'PUT',headers:{{'Content-Type':'application/json'}},
    body:JSON.stringify({{m:[...marks],d:pinData}})}}).catch(function(){{}})
    .finally(function(){{setTimeout(function(){{fbBusy=false;}},400);}});
}}
function fbPull(){{
  if(!syncCode)return;
  fetch(fbPath(syncCode)).then(function(r){{return r.json();}}).then(fbApply).catch(function(){{}});
}}
function fbListen(){{
  if(fbES){{try{{fbES.close();}}catch(_){{}}fbES=null;}}
  if(!syncCode||typeof EventSource==='undefined')return;
  try{{
    fbES=new EventSource(fbPath(syncCode));
    var onchg=function(){{if(!fbBusy)fbPull();}};
    fbES.addEventListener('put',onchg);
...
(function(){{
  ...
  fetch('./data/watch_alerts.json?t='+Date.now()).then(function(r){{return r.ok?r.json():[];}}).then(function(a){{
    ...
    a.forEach(function(x){{
      ...
      var info='';
      if(x.score!=null)info+=' <span class="hs">スコア<b>'+x.score+'</b></span>';
      if(x.ratio)info+=' <span class="hs">相場比'+x.ratio+'</span>';
      if(x.walk!=null)info+=' <span class="hs">駅'+x.walk+'分</span>';
      if(x.area)info+=' <span class="hs">'+Math.round(x.area)+'㎡'+(x.plan?'・'+esc(x.plan):'')+'</span>';
      if(x.tier)info+=' <span class="hs">出口'+x.tier+'</span>';
      if(x.cagr!=null)info+=' <span class="hs">実勢+'+x.cagr+'%/年</span>';
      if(x.net!=null)info+=' <span class="hs">手取り'+(x.net>=0?'+':'−')+Math.abs(x.net).toLocaleString()+'万</span>';
      var tg=(x.tags||[]).map(function(t){{return '<span class="tg">'+esc(t)+'</span>';}}).join('');
      h+='<div class="hit"><b>🔔新着</b> <span class="hk">'+(x.wk==='building'?'🏢':'⭐')+esc(x.watch)+'</span> '
        +(x.name?'<b>'+esc(x.name)+'</b> ':'')+esc(x.loc)
        +'<span class="hp">'+pr+'</span>'+info+' '+tg
        +' <span class="hs">'+ago(x.ts)+'</span> '
