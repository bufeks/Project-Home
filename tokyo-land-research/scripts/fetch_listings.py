...
<p class="meta"><b>このプロジェクトの目的＝「住んで、売る時に“買値以上”で手放せる家」を見つけること。</b>
資産価値が落ちない（できれば値上がりする）＝最高、が狙い。だから安さ単体では選ばず、<b>出口（再売却）で値持ちするか</b>を最重視します。<br>
SUUMO（中古戸建・マンション・土地・新築）から毎日自動収集し、<b>①値持ち（名門立地・出口の堅さ）②割安度（相場比）③駅近 ④将来性 ⑤現地解像度</b>で採点。
<b>プリセット</b>と<b>注目エリアタブ</b>で目的別に絞り込み（予算上限・面積下限は初期適用・変更可）。📌した物件から好みを学習し似た物件を上位表示。<br>
再建築不可・借地・駅徒歩16分以上は既定で除外。旧耐震・古家付きは“除外せず注意タグ＋建替え目安”で表示（名門立地の旧耐震ヴィンテージは値持ちするため過度に減点しない）。<br>
...
<li><b>将来性（再開発）最大+4</b>：区・地区の更新ポテンシャル＝出口の上振れ。</li>
<li><b>減点</b>：再建築不可 −20／借地権 −12／旧耐震 −6（<b>名門立地では −3</b>＝立地が出口を支え値持ちするため半減）／古家付き −2。リスクは消さずタグ・🔎見立て・建替え目安で明示。</li>
</ul>
<ul>
...
<li>狙いは<b>「買値以上で売れる」＝出口S/A × 駅近 ×（割安 または 名門立地の値持ち）</b>。価格の安さだけで選ばない（安い＝出口が弱い/訳ありが多い）。</li>
</ul>
</div></details>
