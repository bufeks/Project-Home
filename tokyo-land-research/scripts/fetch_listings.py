...
<p class="meta"><b>手放す前提＝資産価値が落ちない物件</b>を、SUUMO（中古戸建・マンション・土地・新築）から毎日自動収集。
<b>割安度×将来性×現地解像度</b>で採点し、<b>プリセット</b>と<b>注目エリアタブ</b>で目的別に絞り込み（予算上限・面積下限は初期適用・変更可）。
再建築不可・借地は既定で除外、旧耐震・古家付きは注意タグで表示。<br>
