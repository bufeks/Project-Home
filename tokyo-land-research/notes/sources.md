...
## ソース統合方針（2026-06-18 決定）

一覧（listings.html）の母集団は **SUUMO のみを自動統合**。他サイトは「キュレーション・リンク」として併設する。理由は以下のとおり実地検証済み：

| サイト | fetch | 統合可否 | 検証結果 |
|---|---|---|---|
| **SUUMO** | ✅ | **統合** | 検索結果がサーバーサイド描画。価格・住所・面積・種別を1物件単位で安定抽出できる。 |
| at home | ✅ | ✕→リンク | 一覧がカルーセル/SPA的レイアウト。`ld+json`に物件リスト無し。価格・住所・URL・面積を1物件単位で安定抽出できない。 |
| 健美家 | ✅ | ✕→リンク | 物件詳細URLが取り出せず、価格は別JSON断片にあり住所と結合不可。在庫も投資・再建築不可寄りで「脱ゲテモノ」方針と相性が弱い。 |
| cowcamo | △ | ✕→リンク | 一覧がJS描画でHTMLに物件データが乗らない。 |
| HOME'S / 楽待 | △ | ✕→リンク | アクセス制限・JS描画。 |

- 設計原則：**1物件＝価格＋住所＋面積＋URL を“ズレなく”取れること**を統合の条件とする。これを満たさないソースは、誤データ（例：億→千万の誤表示）を載せるより**リンク併設**に留める。
- 将来 at home 専用パーサを作る選択肢はあるが、サイト改修での定期破損リスクを許容できる場合のみ着手する（現時点は見送り）。
